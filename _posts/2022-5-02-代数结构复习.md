...
$$
S L_{n}(\mathbf{R})=\left\{A \in M_{n}(\mathbf{R}) \mid \operatorname{det}(A)=1\right\}
$$
...
由$S$生成的循环群$\langle a\rangle=\left\{a^{r} \mid r \in \mathbb{Z}\right\}$
...
(2) 如果 $|G|=n$, 则 $G$ 的全部子群为
$\left\{\left\langle a^{d}\right\rangle \mid d\right.$ 为 $n$ 的正因子 $\}$.
...
设 $G$ 是群, $H$ 是 $G$ 的子群. 对任意的 $a \in G$, 群 $G$ 的子集 $a H=\{a h \mid h \in H\} \quad$与 $H a=\{h a \mid h \in H\}$
...
- 高斯整环 $\mathbf{Z}[i]=\{a+bi \mid a, b \in \mathbf{Z}\}$
- 无平方因子$\mathbf{Z}[\sqrt{d}]=\{a+b \sqrt{d} \mid a, b \in \mathbf{Z}\}$
