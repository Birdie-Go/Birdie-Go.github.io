...
| 项目     | 复杂度                                 | 说明                           |
| -------- | -------------------------------------- | ------------------------------ |
| 峰值显存 | $\mathcal{O}(|\theta_{\text{layer}}|)$ | 与层大小成比例，与总参数量无关 |
...
$$
G_t(u,v)=\mathrm{e}^{-t\\mid u-v\\mid ^2},\quad t>0
$$

定义多样性为期望核的对数：

$$
D(f;t)=\log\mathbb{E}_{x,y\sim\mathcal{D}_c}\!\Bigl[\mathrm{e}^{-t\\mid f(x)-f(y)\\mid ^2}\Bigr],\quad t>0
$$
...
$$
\mathcal{D}_{\mathrm{div}}\leftarrow \lbrace y_1\rbrace \cup\Bigl\lbrace y_i\in\mathcal{D}_c\setminus\lbrace y_1\rbrace  \;\Big\mid \; \max_{y_j\in\mathcal{D}_{\mathrm{div}}}G_t\bigl(f(y_i),f(y_j)\bigr)<\tau\Bigr\rbrace 
$$
...
对两篇论文 $\mathcal{P}_1,\mathcal{P}_2$，先让角色分配器 $G_{A_r}$ 决定谁当**基座**（base）、谁当**外援**（source）：
