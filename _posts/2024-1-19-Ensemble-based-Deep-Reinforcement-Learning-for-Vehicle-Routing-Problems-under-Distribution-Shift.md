...
具体来说，配备了 $M$ 个子策略网络，每个策略网络都是一个POMP。策略网络可以表示成 $\pi_\theta$，对于一个大小为 $N$ 的实例，一个策略网络可以得到一组解的轨迹， $\{\tau^1,\tau^2,\cdots,\tau^N\}$ ，并且可以得到其奖励 $R(\tau^i)$。对于一个策略网络，其强化学习的目标是 $\mathcal{L}=\mathbb{E}_{\tau^1 \sim \pi_\theta} R\left(\tau^1\right) \mathbb{E}_{\tau^2 \sim \pi_\theta} R\left(\tau^2\right) \ldots \mathbb{E}_{\tau^N \sim \pi_\theta} R\left(\tau^N\right)$。
