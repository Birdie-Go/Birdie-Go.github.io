...
> **偏好Oracle P** = 判断好坏的外部裁判（可以是人类、GPT-4、规则等）
...
> 想象一个**石头剪刀布**的游戏：
> - 没有绝对的"最强"，只有相互克制
> - 但如果你能找到一种**混合策略**（比如随机出石头、剪刀、布），让对手无论怎么选都无法长期击败你，这就是"纳什均衡"
>
> **von Neumann赢家**就是这个概念在RLHF中的对应物：找到一个AI策略 $\pi^\ast $，使得**无论对手用什么策略 $\pi'$，我至少能打成平手或获胜\ast \ast
>
> 用数学表达就是：
>
...
>
> 意思是：**我选择策略 $\pi$，然后对手选择最针对我的策略 $\pi'$，我要最大化这个最坏情况下的胜率\ast \ast
>
> 直观理解
>
> 想象你在训练一个**辩论AI**：
> - 传统方法：找一个"评分员"给每个回答打分，然后优化分数
> - 新方法：让AI直接和另一个AI"对战"，人类当裁判说谁赢。目标是找到一个AI，**无论对手是谁，人类都更倾向于选我\ast \ast
>
> 这就像训练一个**全能选手**——不是在某一方面最强，而是**没有明显弱点**，面对任何对手都能至少不落下风。
>
> 公式中的 $(\pi^\ast , \pi^\ast )$ 表示**自己打自己**也能达到平衡：
>- 左 $\pi^\ast $：我要最大化我的胜率
> - 右 $\pi^\ast $：对手（也是同样的我）要最小化我的胜率
>
> 当两边都是同一个 $\pi^\ast $ 时达到均衡，意味着这个策略**对自己也是最优的**——没有"自我矛盾"。
>
...
> - 我们让 $\mathbf{y}$ 与**当前水平的自己**（$\pi_t$）比赛
> - 如果 $\mathbf{y}$ 的**胜率**高，就在下一轮给它**更高的权重**（乘以一个大于1的数）
> - 如果胜率低，就给更低权重
>
...
>
> 但直接算那个复杂的**归一化因子**$\log Z$ 很麻烦（要遍历所有可能的答案）。
>
> **聪明的做法**：不精确求解，而是用**最小二乘法**近似——让左边尽量接近右边（3.4中），允许有点小误差。这就像用直线拟合曲线，牺牲一点精度换取计算可行性。

**概率估计**优化目标(3.4)可以用有限样本近似。我们选择为每个提示$\mathbf{x}$采样$K$个回复$\mathbf{y}\_1, \mathbf{y}\_2, \ldots, \mathbf{y}\_K \sim \pi\_t(\cdot \mid \mathbf{x})$，并将经验分布记为$\widehat{\pi}_t^K$。有限样本优化问题可以近似为：
...
**定理3.1**假设优化问题(3.4)是可实现的。将通过(3.4)获得的策略记为$\pi_t$，混合策略$\bar{\pi}\_T = \frac{1}{T}\sum\_{t=1}^{T} \pi\_t$。通过设置$\eta = \Theta(1/\sqrt{T})$，我们有：
...
> **问题**：真实的胜率需要对无穷多个答案求平均，算不了。
>
> **解决方案**：
> 1. **采样估计**：每个题目只采样 $K$ 个答案来估算胜率（用样本代替总体）
> 2. **避免算归一化项**：直接把那个麻烦的常数替换成 $\eta/2$
>
> 最终得到实用的目标函数（3.6）：
>
>
...
>
> **直观含义**：
> - **胜率 = 50%**（平局）→ 不更新（减1/2后为0）
> - **胜率 > 50%**（赢了）→ 增加这个答案的概率（正向更新）
> - **胜率 < 50%**（输了）→ 减少这个答案的概率（负向更新）
>
...
**输入**：基础策略$\pi\_{\theta\_1}$，偏好oracle $\mathbb{P}$，学习率$\eta$，生成样本数$K$

**for**$t = 1, 2, \ldots$ **do\ast \ast
...
**局限性**理论上，通过回归近似最优策略依赖于模型类具有足够表达能力和生成数据良好覆盖输入空间的假设。用常数近似对数配分因子仅在接近软价值函数时有助于减少方差。实验在单个数据集UltraFeedback上运行，由于计算资源有限，模型仅在少数基准上测试，但如果有更多，所提出的方法可以在更多模型、数据集和基准上进一步验证以获得全面评估。
